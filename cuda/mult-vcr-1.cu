...
__global__ void multVectorsKernel(bigint *in1, const bigint *in2,
  ...
  if (tid < n) {
    ...
    bigint_wide o = {0};
    for (int i = 0; i < N - 1; i++) {
      multiply_raw_device<uint32_t, 8>(i1, i2, o);
      i1 = get_256_bit_result(o);
    }
    multiply_raw_device<uint32_t, 8>(i1, i2, out[tid]);
  }
}
...
extern "C" int multiply_test(bigint in1[], const bigint in2[], bigint_wide *out,
  ...
  try {
    ...
    mult_vectors<uint32_t, 1>(in1, in2, out, n);
    return CUDA_SUCCESS;
...
extern "C" int multiply_bench(bigint in1[], const bigint in2[],
  ...
  try {
    ...
    //                             reinterpret_cast<const bigint64 *>(in2),
    //                             reinterpret_cast<bigint64_wide *>(out), n);
    mult_vectors<uint32_t, 500>(in1, in2, out, n);
    return CUDA_SUCCESS;
  } catch (const std::runtime_error &ex) {
    return -1;
  }
