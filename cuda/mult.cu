...
static __device__ __forceinline__ void mul_n(uint32_t *acc, const uint32_t *a,
                                             uint32_t bi) {
#pragma unroll
...
static __device__ __forceinline__ void cmad_n(uint32_t *acc, const uint32_t *a,
                                              uint32_t bi, size_t n = TLC) {
...
static __device__ __forceinline__ void mad_row(uint32_t *odd, uint32_t *even,
...
multiply_raw_device(const bigint &as, const bigint &bs, bigint_wide &rs) {
...
// times
template <int N>
__global__ void multVectorsKernel(bigint *in1, const bigint *in2,
